class ShiftsController < ApplicationController
  ...
  def index
    ...
    if current_user.role? :manager
      ...
      @this_weeks_shifts = Shift.for_next_days(3).chronological.select{|s| s.assignment.store == curr_a.store}#.paginate(:page => params[:page], :per_page => 10)
      @past_weeks_shifts = Shift.for_past_days(4).chronological.select{|s| s.assignment.store == curr_a.store}#.paginate(:page => params[:page], :per_page => 10)
    elsif current_user.role? :admin
      @todays_shifts = Shift.for_next_days(0).chronological#.paginate(:page => params[:page], :per_page => 10)
      @this_weeks_shifts = Shift.for_next_days(3).chronological#.paginate(:page => params[:page], :per_page => 10)
      @past_weeks_shifts = Shift.for_past_days(4).chronological#.paginate(:page => params[:page], :per_page => 10)
    else
      @todays_shifts = Shift.for_employee(e.id).for_next_days(0)
      @this_weeks_shifts = Shift.for_employee(e.id).for_next_days(3)
      @past_weeks_shifts = Shift.for_employee(e.id).for_past_days(4)
    end
  end
